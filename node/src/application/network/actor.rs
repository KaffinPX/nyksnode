...
use crate::application::network::stack::NYKS_PROTOCOL_STR;
...
impl NetworkActor {
    ...
    const KADEMLIA_FOR_NEPTUNE_STRING: &str = concatcp!(NYKS_PROTOCOL_STR, "kad/1.0.0");
    ...
    pub(crate) fn protocol_version(network: Network) -> String {
        format!("{NYKS_PROTOCOL_STR}-{network}")
    }
    ...
    async fn handle_swarm_event(
        ...
        match event {
            ...
            //    leads, keeping the Nyks network healthy and fast.
    ...
    /// Nyks network through our own routing table.
    ///
    ...
    ///    [`PeerMessage`](nyks_p2p::peer::PeerMessage) stream using the
    ...
    ///
    /// Nyks disallows application-level protocol negotiation (the
    ...
    ///
    /// Nyks has a strict policy of disallowing application-level protocol
