...
use crate::transaction::utxo::spendable::SpendableUtxo;

/// represents a transaction input
///
/// this is a newtype wrapper around UnlockedUtxo.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxInput(SpendableUtxo);

impl From<SpendableUtxo> for TxInput {
    fn from(unlocked_utxo: SpendableUtxo) -> Self {
        Self(unlocked_utxo)
    }
}

impl From<TxInput> for SpendableUtxo {
...
impl From<TxInputList> for Vec<SpendableUtxo> {
    fn from(list: TxInputList) -> Self {
...
impl TxInputList {
    ...
    pub fn ms_membership_proofs_iter(&self) -> impl IntoIterator<Item = MsMembershipProof> + '_ {
        self.0.iter().map(|u| u.membership_proof()).cloned()
    }
