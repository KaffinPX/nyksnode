...
impl BlockProposal {
    // TODO: maybe over rpc, idk
    pub(crate) fn own_proposal(block: Block) -> Self {
        Self::OwnComposition(block)
    }
    ...
    pub fn expect(&self, msg: &str) -> &Block {
        match self {
            BlockProposal::OwnComposition(block) => block,
    ...
    pub(crate) fn map<T, F: FnOnce(&Block) -> T>(&self, function: F) -> Option<T> {
        match self {
            BlockProposal::OwnComposition(block) => Some(function(block)),
    ...
    pub(crate) fn filter<F: FnOnce(&Block) -> bool>(&self, predicate: F) -> Option<&Block> {
        match self {
            BlockProposal::OwnComposition(block) => {
                if predicate(block) {
                    Some(block)
                } else {
                    None
