...
#[cfg(not(test))]
pub const POW_MEMORY_PARAMETER: usize = 1 << 27;
#[cfg(test)] // Set to smaller value to allow for testing of PoW
pub const POW_MEMORY_PARAMETER: usize = 1 << 10;
...
impl MTree {
    ...
    pub fn build_inplace(leafs: Vec<Digest>, mut internal_nodes: Vec<Digest>) -> Self {
    ...
    // TODO: checkpoint_distance is not really needed anymore...
...
impl<const MERKLE_TREE_HEIGHT: usize> Pow<MERKLE_TREE_HEIGHT> {
    ...
    fn leaf(commitment: Digest, index: u64) -> Digest {
        ...
        MTree::build_inplace(buds, internal_nodes).root()
    }
    ...
    pub fn preprocess(prev_block_digest: Digest) -> GuesserBuffer<MERKLE_TREE_HEIGHT> {
        let bud_prefix = prev_block_digest;
        ...
        //    -> OK because all are initialized in the loop below.
        unsafe {
            ins.set_len(Self::NUM_LEAFS);
        }

        ins.par_iter_mut().enumerate().for_each(|(i, bud)| {
        ...
        let merkle_tree = MTree::build_inplace(leafs, internal_nodes);
    ...
    pub fn guess(
        ...
        if pow_digest > target { None } else { Some(pow) }
    }
