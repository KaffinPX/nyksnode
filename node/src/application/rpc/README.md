...
1. **Add a new operation**
   ...
   * In `nyks_rpc_core::api::ops::RpcMethods`, create a new variant (e.g. `SubmitBlock`).

2. **Create request and response types**

   * In `nyks_rpc_core::model::message`, define a matching pair of structs:
...
3. **Update the RPC API**
   ...
   * In `nyks_rpc_core::api::rpc::RpcApi`, add two async functions:
...
1. **Implement the `_call` method**
   ...
   * In `rpc::server::service::RpcApi`, implement the function with the `_call` suffix.
